...
)
{
	...
	//頂点からカメラへのベクトル
	float3 eye = normalize(eyePosition - position.xyz);	
	//カメラへのベクトルを接空間へ投影する
	Out.eye.x = dot(eye,T);
	Out.eye.y = dot(eye,B);
	Out.eye.z = dot(eye,N);
...
{   
	...
	light = normalize(In.lightDirection.xyz);

	float3 normalColor	= tex2D(normalSampler, In.bumpUV);
	...
	//reflectVector	= reflect(-normalize(In.toEye), normal);
